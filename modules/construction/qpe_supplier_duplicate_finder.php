<?php
/**
 * Find likely Construction Quick Paid Expense duplicates of Supplier Invoices / Payments
...
require_once __DIR__ . '/includes/qpe_supplier_duplicate_helper.php';
...
$companyId = co_supplier_require_company_id($conn);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'hard_remove_qpe') {
    ...
    if (!$isOwnerAdmin) {
        ...
        $error = 'Confirm that you will keep the Supplier Invoice/Payment and remove only the Quick Paid Expense.';
    } else {
        $expenseId = (int)($_POST['expense_id'] ?? 0);
        $billId = (int)($_POST['bill_id'] ?? 0);
        $paymentId = (int)($_POST['payment_id'] ?? 0);
        $overrideLegacy = !empty($_POST['override_legacy_archive']);
        $valid = qpe_co_match_still_valid($conn, $companyId, $expenseId, $billId, $paymentId, $dateWindow);
        if (empty($valid['ok'])) {
            $error = (string)($valid['error'] ?? 'Match is not valid.');
        } else {
            $res = erp_hard_delete_expense_with_journal($conn, $expenseId, $companyId, $userId, 'construction', $overrideLegacy);
...
$loadError = '';
try {
    $matches = qpe_co_find_duplicate_matches($conn, $companyId, $dateWindow);
} catch (Throwable $e) {
    $loadError = $e->getMessage();
}

$pageTitle = 'QPE vs Supplier Duplicates';
require_once __DIR__ . '/includes/construction_layout_header.php';
?>
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
            <div>
                <h4 class="mb-0"><i class="bi bi-intersect me-2"></i>QPE vs Supplier Invoice duplicates</h4>
                <small class="text-muted">Find Construction Quick Paid Expenses that look like the same purchase as a Supplier Invoice and/or Payment.</small>
            </div>
            <div class="d-flex gap-2">
                <a href="reports/supplier_ap_diagnostics.php" class="btn btn-outline-secondary btn-sm">Supplier AP Diagnostics</a>
                <a href="expenses.php" class="btn btn-outline-primary btn-sm">Quick Paid Expenses</a>
            </div>
        </div>

        <div class="alert alert-warning">
            <strong>Suggested action:</strong> Keep the Supplier Invoice / Payment (formal AP). Remove the matching Quick Paid Expense
            <em>and</em> its journal from the ledger (hard undo — no reversal clutter).
            Matches are suggestions only; confirm each case. This tool never mixes Real Estate vendor data.
            <?php if (!$isOwnerAdmin): ?>
        ...
        <div class="card card-round">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h6 class="mb-0">Likely duplicates (±<?= (int)$dateWindow ?> days, same supplier &amp; amount)</h6>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if ((int)$mrow['bill_id'] > 0): ?>
                                            <a href="supplier_invoice_view.php?id=<?= (int)$mrow['bill_id'] ?>"><?= h($mrow['bill_number'] ?: ('Invoice #' . $mrow['bill_id'])) ?></a><br>
                                    ...
                                    <td>
                                        <?php if ((int)$mrow['payment_id'] > 0): ?>
                                            <a href="supplier_payment_view.php?id=<?= (int)$mrow['payment_id'] ?>">Payment #<?= (int)$mrow['payment_id'] ?></a><br>
                                    ...
                                    <td class="text-nowrap">
                                        ...
                                        <?php elseif ($isOwnerAdmin && !empty($mrow['is_legacy_archive'])): ?>
                                            <button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#<?= h($modalId) ?>" title="Historical archive — requires override confirmation">
                                                <i class="bi bi-trash"></i> Remove QPE (archive)
                                            </button>
                                        <?php else: ?>
                    ...
                    <form method="post">
                        ...
                        <div class="modal-body">
                            ...
                            <ul class="small text-muted mb-3">
                                <li>Supplier Invoice / Payment will <strong>not</strong> be changed.</li>
                                <li>Blocked if the QPE journal is bank-reconciled or the period is locked.</li>
                                <li>This cannot be undone.</li>
                            </ul>
                            <?php if (!empty($mrow['is_legacy_archive'])): ?>
                            <div class="alert alert-danger small mb-3">
                                This is a <strong>historical ERP archive</strong> record, normally permanently read-only.
                                Removing it overrides that protection and purges its journal/GL entries from the shared ledger outside the normal audit trail.
                            </div>
                            <div class="form-check mb-2">
                                <input class="form-check-input" type="checkbox" name="override_legacy_archive" value="1" id="override<?= h($modalId) ?>" required>
                                <label class="form-check-label" for="override<?= h($modalId) ?>">
                                    I understand this is a historical archive record and want to override read-only protection to delete it.
                                </label>
                            </div>
                            <?php endif; ?>
                            <div class="form-check">
                                ...
                                <label class="form-check-label" for="confirm<?= h($modalId) ?>">
                                    I confirm this Quick Paid Expense is a duplicate and I will keep the Supplier Invoice/Payment.
                                </label>
                            </div>
                        </div>
