...
$req = $bundle['req'];
if (!inv_material_request_can_view_in_module($conn, $req, $userId, 'construction')) {
  ...
  <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
    ...
    <div class="d-flex gap-2">
      <?php if (has_permission('inventory_requests.view', MODULE_INVENTORY, $conn)): ?>
        <a class="btn btn-primary btn-sm" href="<?= htmlspecialchars(($appBase !== '' ? $appBase : '') . '/modules/inventory/request_view.php?id=' . $requestId, ENT_QUOTES, 'UTF-8') ?>">Open in Inventory</a>
      <?php endif; ?>
      <a class="btn btn-outline-secondary btn-sm" href="my_material_requests.php">My requests</a>
    </div>
  </div>
</div>
