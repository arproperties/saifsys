<?php
/**
 * Construction Module — Add Work Order
 */
...
// Suggest next WO number
$nextWO = $conn->prepare("SELECT COALESCE(MAX(CAST(SUBSTRING(work_order_number, 4) AS UNSIGNED)), 0) + 1 AS n FROM co_work_orders WHERE company_id = ? AND work_order_number LIKE 'WO-%'");
$nextWO->execute([$cid]);
$nextNum = (int)$nextWO->fetchColumn();
$suggestedWO = 'WO-' . str_pad($nextNum, 3, '0', STR_PAD_LEFT);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$err) {
        $chk = $conn->prepare("SELECT 1 FROM co_work_orders WHERE company_id = ? AND work_order_number = ?");
        $chk->execute([$cid, $work_order_number]);
        if ($chk->fetch()) $err = 'Work order number already exists.';
    }

    if (!$err) {
        $stmt = $conn->prepare("INSERT INTO co_work_orders (company_id, project_id, project_contractor_id, work_order_number, description, amount, status, start_date, end_date, notes, created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
        $stmt->execute([$cid, $project_id, $project_contractor_id, $work_order_number, $description, $amount, $status, $start_date, $end_date, $notes ?: null, $userId]);
        header('Location: work_orders.php?project_id=' . $project_id);
        exit;
    }
}

$pageTitle = 'Add Work Order — ' . $project['project_name'];
...
<div class="mb-4">
    ...
    <h1 class="h4 mb-0">Add Work Order</h1>
...
<form method="post" class="card card-round">
    <div class="card-body">
        <div class="row g-3">
            <div class="col-md-4"><label class="form-label">WO Number *</label><input type="text" name="work_order_number" class="form-control" required value="<?= h($_POST['work_order_number'] ?? $suggestedWO) ?>" placeholder="e.g. WO-001"></div>
            ...
            <div class="col-md-3"><label class="form-label">Status</label><select name="status" class="form-select"><option value="draft" <?= ($_POST['status'] ?? 'draft') === 'draft' ? 'selected' : '' ?>>Draft</option><option value="issued" <?= ($_POST['status'] ?? '') === 'issued' ? 'selected' : '' ?>>Issued</option><option value="in_progress" <?= ($_POST['status'] ?? '') === 'in_progress' ? 'selected' : '' ?>>In Progress</option><option value="completed" <?= ($_POST['status'] ?? '') === 'completed' ? 'selected' : '' ?>>Completed</option><option value="cancelled" <?= ($_POST['status'] ?? '') === 'cancelled' ? 'selected' : '' ?>>Cancelled</option></select></div>
        ...
        <div class="mt-3"><button type="submit" class="btn btn-primary">Save Work Order</button></div>
    </div>
</form>
