...
if ($tablesReady) {
    ...
    $vatCfg = co_bank_reco_vat_config($conn, $cid);
}

$pageTitle = 'Reconcile Bank Account';
...
<div class="co-breco-hero">
  <div class="d-flex flex-wrap align-items-start gap-3">
    <div class="flex-grow-1">
      ...
      <div class="text-muted small">Review each statement line on the left. Match, create (Who / What / Why), or discuss on the right — then click OK to reconcile.</div>
    ...
    <div class="d-flex flex-wrap gap-2">
      ...
      <a href="reports/general_ledger.php" class="btn btn-outline-secondary btn-sm">GL</a>
    </div>
  </div>
</div>

<?php if (!$tablesReady): ?>
  <div class="alert alert-warning">Bank reconciliation tables are not installed. Run <code>migrations/construction_bank_reconciliation.sql</code> and <code>migrations/construction_bank_reconciliation_v2.sql</code>.</div>
<?php elseif (!$banks): ?>
  <div class="alert alert-warning">No active bank accounts. <a href="bank_reconciliation_accounts.php">Configure accounts</a>.</div>
<?php else: ?>
...
<div id="coBankRecoApp" data-csrf="<?= h($csrf) ?>" data-ajax-base="ajax/" data-bank-id="<?= (int) $selectedBank ?>">
    ...
    <div class="co-breco-panel">
      <div class="co-breco-panel-head">
        <ul class="nav nav-tabs card-header-tabs" id="actionTabs">
          ...
          <li class="nav-item"><button type="button" class="nav-link" data-tab="transfer">Transfer</button></li>
          <li class="nav-item"><button type="button" class="nav-link" data-tab="discuss">Discuss</button></li>
          <li class="nav-item"><button type="button" class="nav-link" data-tab="find">Find &amp; Match</button></li>
        </ul>
      </div>
...
window.CO_BRECO_PROJECTS = <?= json_encode($projects) ?>;
window.CO_BRECO_CONTACTS = <?= json_encode($contacts) ?>;
window.CO_BRECO_BANKS = <?= json_encode(array_map(static fn($b) => [
    'id' => (int) $b['id'],
    'label' => ($b['account_no'] ?? '') . ' — ' . ($b['name'] ?? ''),
], $banks)) ?>;
window.CO_BRECO_VAT = <?= json_encode([
...
<script src="assets/bank_reconciliation.js?v=20260703p5"></script>
