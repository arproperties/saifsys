...
| ERP expenses / Quick Paid | `expenses.php` / `expense_*.php` | `erp_post_expense` | Construction Quick Paid **COMPLETE — APPROVED (2026-07-27):** Dr expense + Dr **2130** / Cr bank\|cash\|credit; AP prohibited. Legacy `legacy_archive=1` read-only. Optional informational `co_supplier_id`; optional `project_id` for project cost. See [`CO_QUICK_PAID_EXPENSES_COMPLETE.md`](../construction/CO_QUICK_PAID_EXPENSES_COMPLETE.md). | `erp_expense` | Cancel/repost blocked for legacy | Confirmed |
