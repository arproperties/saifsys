...
$canUndo = co_bank_reco_has_permission($conn, 'construction.bank_reconciliation.undo');
        ...
        <?php else: ?>
          <?php foreach ($history as $row): ?>
            ...
            <tr data-match-id="<?= (int) $row['id'] ?>">
              <td class="small"><?= h($row['confirmed_at'] ?? '') ?></td>
              ...
              <td class="small">
                <?= h(trim(($row['journal_number'] ?? 'GL') . ' — ' . ($row['gl_description'] ?: $row['gl_reference'] ?: ''))) ?>
...
(function(){
  ...
  document.querySelectorAll('.btn-undo-match').forEach(function(btn){
    btn.addEventListener('click', async function(){
      ...
      const r = await fetch('ajax/bank_reco_undo.php', {method:'POST', body: p});
