<?php
/**
 * Real Estate Module - Maintenance by Category Report
 * Maintenance requests and costs grouped by category
 */
...
$byCategory = $conn->prepare("
    SELECT 
        COALESCE(mr.category, 'Uncategorized') as category,
        ...
        AVG(mr.cost) as avg_cost_per_request
    FROM re_maintenance_requests mr
    WHERE " . implode(' AND ', $where) . "
    GROUP BY COALESCE(mr.category, 'Uncategorized')
...
$totals['avg_resolution_hours'] = $completedCategories > 0 ? round($totalHours / $completedCategories, 1) : 0;

// Handle export
if ($exportFormat === 'csv') {
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment; filename="maintenance_by_category_' . date('Y-m-d') . '.csv"');
    $output = fopen('php://output', 'w');
    
    fputcsv($output, ['Maintenance by Category Report - ' . date('M d, Y', strtotime($dateFrom)) . ' to ' . date('M d, Y', strtotime($dateTo))]);
    fputcsv($output, []);
    fputcsv($output, ['Category', 'Total Requests', 'Pending', 'In Progress', 'Completed', 'Cancelled', 'Total Cost', 'Avg Cost', 'Avg Resolution (Hours)']);
    ...
    foreach ($byCategory as $row) {
        fputcsv($output, [
            ...
            number_format($row['avg_cost_per_request'] ?: 0, 2),
...
$pageTitle = 'Maintenance by Category Report';
$pageStyles = '@media print { .no-print { display: none; } }';
require_once __DIR__ . '/includes/re_layout_header.php';
?>
        <div class="d-flex justify-content-between align-items-center mb-4 no-print">
            <h1><i class="bi bi-tags"></i> Maintenance by Category Report</h1>
            <div>
        ...
        <!-- Maintenance by Category Table -->
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">Maintenance by Category (<?= count($byCategory) ?>)</h5>
            </div>
            <div class="card-body">
                    ...
                    <table class="table table-bordered table-sm">
                        ...
                        <tbody>
                            <?php if (empty($byCategory)): ?>
                                <tr>
                                    <td colspan="9" class="text-center text-muted">No maintenance data found</td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($byCategory as $row): ?>
                                    <tr>
                                        ...
                                        <td class="text-end"><?= number_format($row['avg_cost_per_request'] ?: 0, 2) ?> AED</td>
