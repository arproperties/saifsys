        ...
        <div class="card"><div class="card-header">Payments Made</div>
            <div class="table-responsive"><table class="table table-sm"><tbody>
                <?php foreach($payments as $p): ?>
                    <tr><td><?=h($p['payment_date'])?></td><td><?=h($p['reference_number']?:'Payment #'.$p['id'])?></td><td class="text-end"><?=m($p['amount_allocated'])?></td><td><?php if($p['journal_id']): ?><a href="journal_entry_view.php?id=<?=$p['journal_id']?>">Journal</a><?php endif; ?></td></tr>
                <?php endforeach; ?>
                <?php if(!$payments): ?><tr><td class="text-muted">No payments allocated.</td></tr><?php endif; ?>
            </tbody></table></div>
        </div>
        <div class="card mt-3">
...
<div class="alert alert-info no-print mt-3">This posted bill has payments allocated or advances applied. Reverse/unallocate payments and unapply advances before voiding/amending the bill.</div>
<?php endif; ?>
