<?php
/**
 * Construction Module — Edit Project
 */
...
$stmt = $conn->prepare("SELECT * FROM co_projects WHERE id = ? AND company_id = ?");
$stmt->execute([$id, $cid]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$project_code || !$project_name) $err = 'Project code and name are required.';
        ...
        $chk = $conn->prepare("SELECT 1 FROM co_projects WHERE company_id = ? AND project_code = ? AND id != ?");
        $chk->execute([$cid, $project_code, $id]);
        if ($chk->fetch()) $err = 'Project code already exists.';
    }

    if (!$err) {
        $stmt = $conn->prepare("
            UPDATE co_projects SET project_code=?, project_name=?, project_type=?, location=?, start_date=?, expected_completion_date=?, project_manager_id=?, status=?, approved_budget=?, contractor_contract_value=?, client_id=?, contract_value=?, notes=?
            WHERE id=? AND company_id=?
        ");
        $stmt->execute([$project_code, $project_name, $project_type, $location ?: null, $start_date, $expected_completion_date, $project_manager_id, $status, $approved_budget, $contractor_contract_value, $client_id, $contract_value, $notes ?: null, $id, $cid]);
        // When project has exactly one contractor, keep project-contractor contract_value in sync with project's contractor_contract_value
        if ($contractor_contract_value !== null) {
            $one = $conn->prepare("SELECT id FROM co_project_contractors WHERE project_id = ? AND company_id = ?");
            $one->execute([$id, $cid]);
            $rows = $one->fetchAll(PDO::FETCH_ASSOC);
            if (count($rows) === 1) {
                $up = $conn->prepare("UPDATE co_project_contractors SET contract_value = ? WHERE id = ? AND company_id = ?");
                $up->execute([$contractor_contract_value, (int)$rows[0]['id'], $cid]);
            }
        }
...
<div class="mb-4">
    <a href="project_view.php?id=<?= $id ?>" class="btn btn-outline-secondary btn-sm mb-2"><i class="bi bi-arrow-left"></i> Back</a>
...
<form method="post" class="card card-round">
    <div class="card-body">
        <div class="row g-3">
            ...
            <div class="col-md-4"><label class="form-label">Project Manager</label><select name="project_manager_id" class="form-select"><option value="">— Select —</option><?php foreach ($employees as $e): ?><option value="<?= (int)$e['id'] ?>" <?= ((int)($_POST['project_manager_id'] ?? 0)) === (int)$e['id'] ? 'selected' : '' ?>><?= h($e['full_name']) ?></option><?php endforeach; ?></select><?php if (empty($employees)): ?><small class="text-muted d-block mt-1">No employees found. Add employees in HR for this company.</small><?php endif; ?></div>
            ...
            <div class="col-md-4" id="client-block"><label class="form-label">Client (CLIENT projects)</label><select name="client_id" class="form-select"><option value="">— Select —</option><?php foreach ($clients as $c): ?><option value="<?= (int)$c['id'] ?>" <?= ((int)($_POST['client_id'] ?? 0)) === (int)$c['id'] ? 'selected' : '' ?>><?= h($c['client_name']) ?></option><?php endforeach; ?></select></div>
            <div class="col-md-4" id="contract-value-block"><label class="form-label">Contract Value (AED)</label><input type="number" step="0.01" name="contract_value" class="form-control" value="<?= h($_POST['contract_value'] ?? '') ?>"><small class="text-muted d-block mt-1">Amount client will pay you</small></div>
