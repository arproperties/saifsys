...
function co_bank_xlsx_col_to_index(string $letters): int
{
...
function co_bank_xlsx_index_to_col(int $index): string
{
    $index++;
    $col = '';
    while ($index > 0) {
...
function co_bank_excel_serial_to_ymd(float $serial): ?string
{
    if ($serial < 1) {
        return null;
    }
...
/** @return list<string> */
function co_bank_xlsx_load_shared_strings(ZipArchive $zip): array
{
...
function co_bank_xlsx_first_sheet_path(ZipArchive $zip): ?string
{
 ...
 */
function co_bank_xlsx_read_grid(string $path): array
{
    ...
    $shared = co_bank_xlsx_load_shared_strings($zip);
    $sheetPath = co_bank_xlsx_first_sheet_path($zip);
    ...
    foreach ($doc->sheetData->row as $row) {
        ...
        foreach ($row->c as $cell) {
            ...
            $colIdx = co_bank_xlsx_col_to_index($m[1]);
            $type = (string) ($cell['t'] ?? '');
            $value = '';
            if ($type === 's') {
 ...
 */
function co_bank_parse_statement_xlsx_native(string $path): array
{
    if (!function_exists('co_bank_resolve_import_columns')) {
        require_once __DIR__ . '/construction_bank_reconciliation.php';
    }

    try {
        $grid = co_bank_xlsx_read_grid($path);
    } catch (Throwable $e) {
    ...
    for ($r = 1; $r <= $scanTo; $r++) {
        ...
        $map = co_bank_resolve_import_columns(array_values($headers));
        if ($map) {
        ...
        foreach ($colMap as $field => $idx) {
            ...
            if ($field === 'date') {
                if (is_numeric($raw) && (float) $raw > 1000) {
                    $cells[$idx] = co_bank_excel_serial_to_ymd((float) $raw) ?? (string) $raw;
                } else {
                    $cells[$idx] = co_bank_normalize_import_date($raw) ?? (string) $raw;
                }
            } else {
...
function co_bank_import_template_xlsx_path(): string
{
    return dirname(__DIR__) . '/templates/bank_statement_import_template.xlsx';
}

function co_bank_import_template_xlsx_exists(): bool
{
    $path = co_bank_import_template_xlsx_path();
    return is_file($path) && filesize($path) > 100;
}

function co_bank_serve_import_template_xlsx(): void
{
    $path = co_bank_import_template_xlsx_path();
    if (!co_bank_import_template_xlsx_exists()) {
        co_bank_build_import_template_xlsx_file($path);
    }
    if (!is_file($path)) {
...
function co_bank_build_import_template_xlsx_file(string $destPath): bool
{
    if (!class_exists('ZipArchive')) {
        return false;
    }
    ...
    $samples = [
        ['30-06-2026', 'OUTWARD CLEARING CHQ DEPOSIT', '26181026972519190282', '1500.00', '0.00'],
        ['30-06-2026', 'OUTWARD CLEARING CHQ DEPOSIT', '26181026972519190282', '1500.00', '0.00'],
        ['25-06-2026', 'BANKNET TRANSFER', '26181026972519190282', '0.00', '10879.00'],
        ['25-06-2026', 'VALUE ADDED TAX', '--', '0.00', '5.00'],
    ];
    ...
    foreach ($allRows as $row) {
        ...
        foreach ($row as $i => $val) {
            $col = co_bank_xlsx_index_to_col((int) $i);
            if ($rowNum === 1) {
        ...
        $sheetRows .= '<row r="' . $rowNum . '" spans="1:5">' . $cells . '</row>';
    }
 ...
 */
function co_bank_try_load_phpspreadsheet(): bool
{
    ...
    try {
        ...
        error_log('co_bank_try_load_phpspreadsheet: ' . $e->getMessage());
        return false;
    }
