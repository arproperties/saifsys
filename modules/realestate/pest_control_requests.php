<?php
/**
 * Real Estate — Tenant pest control requests (management view). Also linked from Tenant extra service requests.
 */
...
try {
    ...
    $pageTitle = 'Pest control requests';
    require_once __DIR__ . '/includes/re_layout_header.php';
    echo '<div class="alert alert-info">No pest control requests yet. Run migration <code>tenant_cleaning_pest_control.sql</code> if needed.</div>';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id && in_array($action, ['approve', 'reject'], true)) {
        ...
        $stmt = $conn->prepare("
            UPDATE tenant_pest_control_requests
        ...
        if ($stmt->rowCount()) {
            // Tenant in-app notification: pest control request status changed
            require_once __DIR__ . '/../../includes/tenant_notifications.php';
            try {
                $pcLease = $conn->prepare("SELECT lease_id FROM tenant_pest_control_requests WHERE id = ? AND company_id = ?");
                $pcLease->execute([$id, $currentCompanyId]);
                $pcLeaseId = (int)($pcLease->fetchColumn() ?: 0);
                tenant_notification_create($conn, [
                    ...
                    'entity_id' => $id,
                    'title' => 'Pest control request ' . $status,
                    'body' => 'Your pest control request has been ' . $status . '.',
                ]);
            } catch (Throwable $e) {
                error_log('pest notification failed: ' . $e->getMessage());
            }
            if ($status === 'approved') {
                $q = $conn->prepare("
                ...
                if ($row && !empty($row['email'])) {
                    ...
                    $subject = 'Pest control service confirmation';
                    $html = "
                    <html><body style='font-family: Arial, sans-serif; line-height:1.6;'>
                        <p>Dear " . htmlspecialchars($tenantName ?: 'Tenant', ENT_QUOTES, 'UTF-8') . ",</p>
                        <p>Your pest control request has been <strong>approved</strong>.</p>
            ...
            $_SESSION['pest_flash'] = ['success', $action === 'approve' ? 'Request approved.' : 'Request rejected.'];
        }
        header('Location: pest_control_requests.php');
        exit;
    }
}

$flash = $_SESSION['pest_flash'] ?? null;
unset($_SESSION['pest_flash']);
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div class="page-header-label">Tenant pest control requests</div>
    <div>
        <a href="pest_control_rates.php" class="btn btn-outline-primary me-2">Pest control rates</a>
        <a href="extra_service_requests.php" class="btn btn-secondary me-2">Extra services</a>
        <a href="cleaning_requests.php" class="btn btn-outline-secondary">Cleaning</a>
    </div>
</div>
...
<div class="card card-round">
    <div class="card-body">
        <?php if (empty($requests)): ?>
            <p class="text-muted mb-0">No pest control requests found.</p>
        <?php else: ?>
                    ...
                    <tbody>
                        <?php foreach ($requests as $r): ?>
                            <tr>
                                ...
                                <td><?= h(ucwords(str_replace('_', ' ', $r['unit_type']))) ?></td>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Pest control request #<?= (int)$r['id'] ?></h5>
                    ...
                    <div class="modal-body text-start">
                        ...
                        <p><strong>Unit type:</strong> <?= h(ucwords(str_replace('_', ' ', $r['unit_type']))) ?> | <strong>Amount (AED):</strong> <?= $r['total_amount_aed'] !== null ? number_format((float)$r['total_amount_aed'], 2) : '—' ?></p>
