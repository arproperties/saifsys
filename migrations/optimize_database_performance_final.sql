-- Database Performance Optimization Script (Final Corrected)
...
-- Email system indexes (using correct column names)
CREATE INDEX IF NOT EXISTS idx_email_log_entity_type ON email_log(entity_type);
CREATE INDEX IF NOT EXISTS idx_email_log_entity_id ON email_log(entity_id);
...
-- Credit notes indexes (using correct column names)
...
CREATE INDEX IF NOT EXISTS idx_credit_notes_total ON credit_notes(total_amount);

-- Refunds indexes (using correct column names)
