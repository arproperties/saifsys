<?php
/**
 * Tenant Portal — Pay for pest control (Cash at Office)
...
$id = (int)($_GET['id'] ?? 0);
if (!$id) {
    header('Location: pest_control.php');
    exit;
}
...
$selectCols = "id, unit_type, total_amount_aed, status";
$hasCashColumn = false;
try {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $cashPaymentTablesExist && !$cashRequestId) {
    csrf_verify();
    $create = cash_payment_create_request($conn, $company_id, 'pest_control', $requestId, $amount);
    if ($create) {
        cash_payment_link_service_request($conn, 'pest_control', $requestId, $create['id']);
        header('Location: pay_pest_control.php?id=' . $requestId . '&cash=1');
        exit;
    }
}
...
if ($hasCashColumn && ($_GET['cash'] ?? '') === '1' && file_exists(__DIR__ . '/../modules/realestate/includes/cash_payment_helper.php')) {
    ...
    if (cash_payment_tables_exist($conn)) {
        $q = $conn->prepare("SELECT r.id, r.request_number, r.status, r.expires_at FROM re_cash_payment_requests r INNER JOIN tenant_pest_control_requests p ON p.cash_payment_request_id = r.id WHERE p.id = ? AND r.company_id = ?");
...
$pageTitle = 'Pay for pest control';
require_once __DIR__ . '/includes/tenant_layout_header.php';
?>

<h4 class="page-title">Pay for pest control</h4>
<div class="portal-card card">
    <div class="card-body">
        <p class="mb-2">Pest control request <strong>#<?= $requestId ?></strong> — <?= h(ucwords(str_replace('_', ' ', $row['unit_type']))) ?></p>
        ...
        <?php if ($cashRequest): ?>
            <div class="alert alert-info mb-3">
                ...
                <?php elseif ($cashRequest['status'] === 'paid_verified'): ?>
                    <p class="mb-0 small">Payment verified. Your pest control service can be scheduled.</p>
                <?php endif; ?>
            </div>
            <a href="pest_control.php" class="btn btn-outline-primary">Back to Pest control</a>
        <?php else: ?>
            ...
            <a href="pest_control.php" class="btn btn-link mt-2">Back to Pest control</a>
        <?php endif; ?>
    </div>
</div>
