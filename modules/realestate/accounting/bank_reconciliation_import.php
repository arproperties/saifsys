<?php
/**
 * Real Estate — Import bank statement (preview + confirm, native XLSX).
 */
...
re_bank_reco_require_permission($conn, 'realestate.bank_reconciliation.import');
...
<?php elseif (!$banks): ?>
  <div class="alert alert-warning">No bank accounts configured. <a href="bank_accounts.php">Add a bank account</a>.</div>
<?php else: ?>
  ...
  <div class="card-body">
    <div class="row g-3 align-items-end">
      ...
      <div class="col-md-5">
        ...
        <div class="form-text">
          <a href="ajax/re_bank_reco_import_template.php?v=20260724" class="fw-semibold">Download Excel template (.xlsx)</a>
          <br>Columns: <strong>date</strong> (DD-MM-YYYY), <strong>description</strong>, <strong>reference</strong>, <strong>Debit</strong>, <strong>Credit</strong>, <strong>Balance</strong>
          — or use a single <strong>amount</strong> column (+/-) instead of Debit/Credit.
          <br><span class="text-muted"><strong>Balance</strong> is the bank running balance after each line (needed for Statement balance on the reconciliation report).</span>
        </div>
      </div>
      <div class="col-md-3">
  ...
  <div class="card-body">
    ...
    <div class="table-responsive" style="max-height:360px">
      <table class="table table-sm table-striped mb-0">
        <thead class="table-light sticky-top"><tr><th>Date</th><th class="text-end">Debit</th><th class="text-end">Credit</th><th class="text-end">Amount</th><th class="text-end">Balance</th><th>Description</th><th>Ref</th></tr></thead>
...
(function(){
  ...
  document.getElementById('btnPreview').addEventListener('click', async function(){
    ...
    try {
      ...
      fd.append('file', f);
      const res = await fetch(ajaxBase + 're_bank_reco_import_preview.php', {method:'POST', body:fd});
      const text = await res.text();
      let j;
      try { j = JSON.parse(text); } catch (parseErr) {
        console.error('Preview response:', text);
        alert('Import preview failed (server error). Try CSV format or check PHP error log.');
        return;
      }
      ...
      (j.preview_rows || []).forEach(function(r){
        ...
        const bal = (r.running_balance != null && r.running_balance !== '') ? Number(r.running_balance).toFixed(2) : '—';
        tr.innerHTML = '<td>'+esc(r.txn_date)+'</td><td class="text-end">'+(debit > 0 ? debit.toFixed(2) : '—')+'</td><td class="text-end">'+(credit > 0 ? credit.toFixed(2) : '—')+'</td><td class="text-end">'+amt.toFixed(2)+'</td><td class="text-end">'+bal+'</td><td>'+esc(r.description)+'</td><td>'+esc(r.reference||'')+'</td>';
        tb.appendChild(tr);
      });
  ...
  document.getElementById('btnConfirm').addEventListener('click', async function(){
    ...
    fd.append('_csrf', CSRF);
    fd.append('bank_account_id', document.getElementById('bankSel').value);
    const res = await fetch(ajaxBase + 're_bank_reco_import_confirm.php', {method:'POST', body:fd});
