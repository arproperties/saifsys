<?php
/**
 * Construction Module — Project Cost Summary Report
 */
...
$stmt = $conn->prepare("
    SELECT p.id, p.project_code, p.project_name, p.project_type, p.status, p.approved_budget,
...
foreach ($projects as $p) {
    $budget = (float)($p['approved_budget'] ?? 0);
    $actual = (float)$p['total_cost'];
    $exportRows[] = [
        ...
        'budget' => $budget,
        'actual_cost' => $actual,
        'variance' => $budget > 0 ? $actual - $budget : 0,
    ];
}
if (($_GET['export'] ?? '') === 'excel') {
    accounting_export_excel_or_csv($exportRows, ['project' => 'Project', 'type' => 'Type', 'status' => 'Status', 'budget' => 'Budget', 'actual_cost' => 'Actual Cost', 'variance' => 'Variance'], 'construction_project_cost_summary', 'Construction Project Cost Summary');
} elseif (($_GET['export'] ?? '') === 'csv') {
    accounting_export_csv($exportRows, ['project' => 'Project', 'type' => 'Type', 'status' => 'Status', 'budget' => 'Budget', 'actual_cost' => 'Actual Cost', 'variance' => 'Variance'], 'construction_project_cost_summary');
}

$pageTitle = 'Project Cost Summary';
require_once __DIR__ . '/../includes/construction_layout_header.php';
?>

<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2 no-print">
    <div>
        <h1 class="h4 mb-0">Project Cost Summary</h1>
        <p class="text-muted mb-0">Total cost by project, including supplier invoices, contractor payments, direct project costs, and historical ERP expenses.</p>
    </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover mb-0">
                <thead class="table-light">
                    <tr><th>Project</th><th>Type</th><th>Status</th><th>Budget</th><th>Actual Cost</th><th>Variance</th></tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="<?= $variance > 0 ? 'text-danger' : '' ?>"><?= co_format_money($variance) ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
