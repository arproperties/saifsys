<?php
/**
 * Construction Accounting — View Journal Entry
...
function co_load_journal_header(PDO $conn, int $journalId, int $companyId): ?array
{
    $stmt = $conn->prepare("
        SELECT jh.*,
...
function co_load_journal_lines(PDO $conn, int $journalId): array
{
    $stmt = $conn->prepare("
...
if (!$journal) {
    $_SESSION['error'] = 'Journal entry not found for the current company.';
    header('Location: journal_entry_list.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'submit_approval' && !$journal['is_posted']) {
    ...
    if ($action === 'approve' && !$journal['is_posted']) {
...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
    ...
    <div>
        ...
        <?php if (!$journal['is_posted']): ?>
            <a href="journal_entry_add.php?id=<?= (int)$journalId ?>" class="btn btn-outline-info">
    ...
    <div class="card-body">
        ...
        <div class="row mt-2">
            <div class="col-md-3">
                ...
                <span class="h5 text-primary"><?= number_format((float)$journal['total_debit'], 2) ?> AED</span>
            </div>
            <div class="col-md-3">
                <strong>Total Credit:</strong><br>
                <span class="h5 text-primary"><?= number_format((float)$journal['total_credit'], 2) ?> AED</span>
            </div>
            <div class="col-md-3">
                ...
                <tbody>
                    <?php foreach ($journalLines as $line): ?>
                        <tr>
                            ...
                            <td class="text-end">
                                <?php if ((float)$line['debit_amount'] > 0): ?>
                                    <strong><?= number_format((float)$line['debit_amount'], 2) ?></strong>
                                <?php else: ?>
                                    <span class="text-muted">-</span>
                                <?php endif; ?>
                            </td>
                            <td class="text-end">
                                <?php if ((float)$line['credit_amount'] > 0): ?>
                                    <strong><?= number_format((float)$line['credit_amount'], 2) ?></strong>
                                <?php else: ?>
                                    <span class="text-muted">-</span>
                                <?php endif; ?>
                            </td>
