...
re_bank_reco_require_permission($conn, 'realestate.bank_reconciliation.cash_coding');
...
(function(){
  ...
  function contactOptions(selected){
    ...
    [['tenants','tenant','Tenants'],['vendors','vendor','Vendors']].forEach(function(g){
  ...
  function accountOptions(selected){
    return '<option value="">—</option>' + (window.RE_BRECO_ACCOUNTS||[]).map(function(a){
      return '<option value="'+a.id+'"'+(String(a.id)===String(selected)?' selected':'')+'>'+esc(a.account_code+' — '+a.account_name)+'</option>';
    }).join('');
  }

  function typeOptions(selected){
    const types = [['quick_expense','Quick expense'],['bank_charge','Bank charge'],['tenant_receipt','Tenant receipt'],['cash_withdrawal','Cash withdrawal']];
  ...
  async function loadLines(){
    ...
    const r = await fetch('ajax/re_bank_reco_cash_coding_list.php?'+q);
  ...
  document.getElementById('btnBulkReconcile').addEventListener('click', async function(){
    ...
    const r = await fetch('ajax/re_bank_reco_cash_coding_bulk.php', {method:'POST', body:p});
    const j = await r.json();
