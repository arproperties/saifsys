<?php
/**
 * Construction Module — Edit Contractor Document
 */
...
$stmt = $conn->prepare("SELECT d.*, c.contractor_name FROM co_documents d JOIN co_contractors c ON c.id = d.contractor_id WHERE d.id = ? AND d.company_id = ?");
$stmt->execute([$id, $cid]);
$doc = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$doc) { header('Location: contractors.php'); exit; }
$contractor_id = (int)$doc['contractor_id'];
...
$pageTitle = 'Edit Contractor Document — ' . $doc['contractor_name'];
...
<div class="mb-4">
    ...
    <h1 class="h4 mb-0">Edit Contractor Document</h1>
