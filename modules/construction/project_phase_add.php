<?php
/**
 * Construction Module — Add Project Phase
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$err) {
        $stmt = $conn->prepare("
            INSERT INTO co_project_phases (company_id, project_id, phase_name, sequence, planned_start_date, planned_end_date, actual_start_date, actual_end_date, percent_complete, status, notes)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->execute([$cid, $project_id, $phase_name, $sequence, $planned_start_date, $planned_end_date, $actual_start_date, $actual_end_date, $percent_complete, $status, $notes ?: null]);
        header('Location: project_phases.php?project_id=' . $project_id);
        exit;
    }
}

$pageTitle = 'Add Phase — ' . $project['project_name'];
...
<form method="post" class="card card-round">
    <div class="card-body">
        <div class="row g-3">
            <div class="col-md-8"><label class="form-label">Phase Name *</label><input type="text" name="phase_name" class="form-control" required value="<?= h($_POST['phase_name'] ?? '') ?>" placeholder="e.g. Excavation, Structure, MEP, Finishing"></div>
        ...
        <div class="mt-3"><button type="submit" class="btn btn-primary">Save Phase</button></div>
    </div>
</form>
