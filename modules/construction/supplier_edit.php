<?php
/**
 * Construction Module — Edit Supplier (Expenses Phase 1)
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$err) {
        $stmt = $conn->prepare("
            UPDATE co_suppliers SET supplier_name=?, contact_person=?, email=?, phone=?, address=?, tax_number=?,
                vat_number=?, bank_name=?, account_number=?, iban=?, swift_code=?, bank_details=?, notes=?, is_active=?
            WHERE id=? AND company_id=?
        ");
        $stmt->execute([$supplier_name, $contact_person ?: null, $email ?: null, $phone ?: null, $address ?: null, $tax_number ?: null, $vat_number ?: null, $bank_name ?: null, $account_number ?: null, $iban ?: null, $swift_code ?: null, $bank_details ?: null, $notes ?: null, $is_active, $id, $cid]);
        header('Location: supplier_view.php?id=' . $id);
        exit;
    }
} else {
    $_POST = $s;
    $_POST['tax_vat_number'] = !empty($s['vat_number']) ? $s['vat_number'] : ($s['tax_number'] ?? '');
}
...
<div class="mb-4">
    <a href="supplier_view.php?id=<?= $id ?>" class="btn btn-outline-secondary btn-sm mb-2"><i class="bi bi-arrow-left"></i> Back</a>
