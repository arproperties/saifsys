-- =====================================================
-- Fix Mobile User to Link to Client Table (Version 2)
...
-- Step 4: Migrate existing customers to client table (if they don't exist)
INSERT INTO `client` (
...
-- Step 5: Update online_bookings to link to client records based on phone
UPDATE `online_bookings` ob
INNER JOIN `client` cl ON cl.mobile_num = ob.customer_phone
SET ob.client_id = cl.id
WHERE ob.client_id IS NULL OR ob.client_id NOT IN (SELECT id FROM `client`);

-- Step 6: Drop old foreign key if exists
ALTER TABLE `online_bookings`
DROP FOREIGN KEY IF EXISTS `fk_booking_customer`;

ALTER TABLE `online_bookings`
DROP FOREIGN KEY IF EXISTS `fk_booking_client`;

-- Step 7: Add new foreign key constraint to client table
ALTER TABLE `online_bookings`
