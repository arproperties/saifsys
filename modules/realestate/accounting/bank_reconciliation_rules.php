...
$csrf = csrf_token();
$tablesReady = re_bank_reco_tables_ready($conn) && re_bank_rules_table_ready($conn);
...
if ($tablesReady) {
    ...
    $rules = re_bank_rules_list($conn, $cid);

    $lineId = (int) ($_GET['line_id'] ?? 0);
    if ($lineId > 0) {
        $line = re_bank_get_statement_line($conn, $lineId, $cid);
        if ($line) {
            $prefill = re_bank_rule_from_line($conn, $cid, $line, (int) $line['bank_account_id']);
...
<?php if (!$tablesReady): ?>
  <div class="alert alert-warning">Run <code>migrations/re_bank_reconciliation_v2.sql</code> to enable bank rules.</div>
<?php else: ?>
  ...
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <form class="modal-content" id="ruleForm">
      ...
      <div class="modal-body row g-2">
        ...
        <div class="col-md-8"><label class="form-label small">Bank account (optional)</label>
        ...
        <div class="col-md-4"><label class="form-label small">Transaction type</label>
          <select name="action_transaction_type" id="f_action_type" class="form-select form-select-sm">
            ...
            <option value="tenant_receipt">Tenant receipt</option>
        ...
        <div class="col-md-12"><label class="form-label small">Account (What)</label>
...
window.RE_BANK_RULES = ' . json_encode($rules) . ';
window.RE_RULE_PREFILL = ' . json_encode($prefill) . ';
(function(){
  function initBankRulesPage(){
    if (typeof bootstrap === "undefined") return;
    const CSRF = ' . json_encode($csrf) . ';
    const rulesById = {};
    (window.RE_BANK_RULES || []).forEach(function(r){ rulesById[String(r.id)] = r; });
    const modalEl = document.getElementById("ruleModal");
    if (!modalEl) return;
    const modal = bootstrap.Modal.getOrCreateInstance(modalEl);
    const contacts = window.RE_BRECO_CONTACTS || {tenants:[],vendors:[]};

    function fillContacts(type, selected){
      const sel = document.getElementById("f_contact_id");
      if (!sel) return;
      sel.innerHTML = "<option value=\"\">— Select —</option>";
      const map = {tenant:"tenants",vendor:"vendors"};
    ...
    document.getElementById("f_contact_type").addEventListener("change", function(){ fillContacts(this.value, ""); });
    ...
    document.querySelectorAll(".btn-del-rule").forEach(function(btn){
      btn.addEventListener("click", async function(){
        ...
        const r = await fetch("ajax/re_bank_reco_rules_delete.php", {method:"POST", body:p});
    ...
    document.getElementById("ruleForm").addEventListener("submit", async function(ev){
      ...
      const r = await fetch("ajax/re_bank_reco_rules_save.php", {method:"POST", body:fd});
      const j = await r.json();
      if (!j.success) { alert(j.error || "Save failed"); return; }
      location.reload();
    });
    if (window.RE_RULE_PREFILL) openForm(window.RE_RULE_PREFILL);
  }
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", initBankRulesPage);
  else initBankRulesPage();
})();
</script>';
?>
