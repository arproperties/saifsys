...
function co_load_company_settings(PDO $conn, int $cid): array {
    $stmt = $conn->prepare("
...
$company = co_load_company_settings($conn, $cid);

function co_handle_document_logo_upload(int $companyId): ?string {
    ...
    $uploadDir = $appRoot . '/uploads/construction_logos';
    if (!is_dir($uploadDir)) {
        if (!@mkdir($uploadDir, 0777, true)) {
            throw new RuntimeException('Cannot create logo upload directory. Please create uploads/construction_logos and make it writable.');
        }
    ...
    if (!is_writable($uploadDir)) {
        throw new RuntimeException('Logo upload directory is not writable. Please set write permission for uploads/construction_logos.');
    }
    ...
    return 'uploads/construction_logos/' . $filename;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $uploadedLogoPath = co_handle_document_logo_upload($cid);
        ...
        co_pdf_save_document_settings($conn, $cid, [
        ...
        $conn->commit();
        $msg = 'Construction PDF document settings saved.';
    } catch (Throwable $e) {
...
<div class="mb-4">
    ...
    <p class="text-muted mb-0">Control the company details shown on Construction tax invoices and receipts for the current company.</p>
</div>
    ...
    <div class="card-body">
        ...
        <div class="row g-2">
            <?php foreach ([
                ...
                'show_bank_details' => 'Bank details on tax invoice',
            ] as $key => $label): ?>
                <div class="col-md-4">
                    <div class="form-check">
        ...
        <div class="mt-4"><button class="btn btn-primary">Save Document Settings</button></div>
    </div>
</form>
